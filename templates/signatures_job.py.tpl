...
bulk_data_table_schemas = requests.get(${controlshift_bulk_data_endpoint}).json()['tables']
signatures_table_columns = next(schema for schema in bulk_data_table_schemas if schema['table']['name'] == 'signatures')['table']['columns']
# TODO: Map data types in the bulk data table schema to data types supported by Glue / Redshift
signatures_column_mapping = [(key, value['sql_type'], key, value['sql_type']) for (key, value) in signatures_table_columns.items()]
applymapping1 = ApplyMapping.apply(
    frame = datasource1,
    mappings = signatures_column_mapping,
